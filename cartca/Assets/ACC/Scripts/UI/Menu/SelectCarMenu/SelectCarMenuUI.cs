...
public class SelectCarMenuUI :WindowWithShopLogic
{
	...
	int CurrentCarIndex { get { return SelectedCar == null ? -1 : CanSelectCars.IndexOf (SelectedCar); } }
	...
	void SelectCar (CarPreset newCar)
	{
		...
		if (newCar.AvailibleColors.Count == 1)
		{
			SelectColorPanelButton.SetActive (false);
			OnSelectParamsPanel ();
		}
		else
		{
			SelectColorPanelButton.SetActive (true);
		}
	}
	...
	void NextCar ()
	{
		SelectCar (CanSelectCars[MathExtentions.LoopClamp (CurrentCarIndex + 1, 0, (CanSelectCars.Count))]);
	}
