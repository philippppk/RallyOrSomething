...
public struct Wheel
{
    ...
    public void UpdateVisual (bool carIsVisible)
    {
        ...
        if (!StopEmitFX && FXController && WheelCollider.isGrounded && CurrentMaxSlip > SlipForGenerateParticle)
        {
            //Emit particle.
            if (carIsVisible)
            {
                var particles = FXController.GetParticles (Hit.collider.gameObject.layer);
